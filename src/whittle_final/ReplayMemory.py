...
class ReplayMemory(object):
    ...
    def sample_trans_Q(self,state,action):
        # returns a single transition sampled randomly from the replay buffer
        temp = [self.memory[i] for i in range(len(self.memory)) if 
            (self.memory[i].state,self.memory[i].action)==(state,action)]
        return random.sample(temp,1)
    
    def sample_trans_whittle(self,state):
        # returns a single transition sampled randomly from the replay buffer
        temp = [self.memory[i] for i in range(len(self.memory)) if 
            (self.memory[i].state,self.memory[i].action)==(state,0)]
        return random.sample(temp,1)
