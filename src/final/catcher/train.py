...
def parse_args():
    ...
    parser.add_argument("--deterministic", type=lambda x: bool(strtobool(x)), default=True, nargs="?", const=True,
        help="True if the environment is deterministic")
    parser.add_argument("--env-id", type=str, default="Catcher-v0",help="the environment id")
    parser.add_argument("--total-gradsteps", type=int, default=500000,
    ...
    # total_timesteps = learning_starts + total_gradsteps
...
if __name__ == "__main__":
    ...
    # env setup
    render = False
    game = Catcher(width=521, height=521, deterministic=args.deterministic)
    game.env_init({'random_seed': 42, 'render': render})
    # actions: idle, left, right
    actions = [0,1,2]

    if args.agent == "FGDQN":
        model = FGDQN(game, actions, device, args)
    elif args.agent == "DQN":
        model = DQN(game, actions, device, args)

    eval_game = Catcher(width=521, height=521, deterministic=args.deterministic)
    eval_game.env_init({'random_seed': 64, 'render': render})
    model.learn(writer,args,eval_env=eval_game)
