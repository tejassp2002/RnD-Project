...
video=cv2.VideoWriter('./video.mp4',fourcc, 20.0,(521,521))
...
class CatcherBase(base.PyGameWrapper):
    ...
    def get_dx_from_action(self, action, deterministic):
        self.dx = 0.0
        if self.render:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
        ...
        if action == 0: # idle
            if deterministic:
                ...
                c = np.random.choice([-1,0,1],p=[(1-p)/2,p,(1-p)/2])
                self.dx += c*self.player_speed
        if action == 1:  # left
            if deterministic:
                self.dx = -self.player_speed
            else:
                c = np.random.choice([-1,0],p=[0.8,1-p])
                self.dx += c*self.player_speed
        if action == 2:  # right
            if deterministic:
                self.dx = self.player_speed
            else:
                c = np.random.choice([0,1],p=[1-p,0.8])
                self.dx += c*self.player_speed
    ...
    def step(self, action, deterministic):
        ...
        self.get_dx_from_action(action, deterministic)
...
class Catcher(CatcherBase):
    ...
    def __init__(self, width=256, height=256, deterministic=False):
    ...
    def env_init(self, env_info={}):
        ...
        self.new_rewards = env_info.get('rewards', {"negative": -1.0, "positive": 1.0})
    ...
    def env_step(self, action):
        ...
        self.step(action,self.deterministic)
    ...
    def getImage(self):
        # pygame.image.save(self.screen, 'eval.jpeg')
        # img = cv2.imread('eval.jpeg', cv2.IMREAD_COLOR)
...
def test_Catcher():
    ...
    print("aadadadad",obs.shape,obs.dtype)
    ...
    while True:
        ...
        if i==20:
            print("DONE")
            # video.release()
        # print(reward_obs_term[0], torch.tensor(reward_obs_term[1]))
        # print(reward_obs_term[1].shape, reward_obs_term[1].dtype)
